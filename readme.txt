...
The Problem:
ADVANCED ARTIFICIAL INTELLIGENCE - (dynamic Bayesian networks)
...
The position Xt of the object at time t depends on its position Xt-1 and the direction of the wind At-1 at the previous time t-1, according to the following table: (Xt table - .xls file)
...
There is a camera that tells us the region where the object is located at each moment in time. The relevant sensor model is given by the following table: (Et table - .xls file)
